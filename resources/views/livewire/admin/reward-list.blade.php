            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 pb-4 border-b border-gray-100">
                <div class="flex w-full md:w-auto">
                    ...
                    <div class="relative w-full sm:w-64">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-brand focus:border-brand block w-full pl-10 p-2.5" placeholder="Cari nama hadiah...">
                    </div>
                </div>
            ...
            <div class="overflow-x-auto mt-4">
                <table class="w-full text-sm text-left text-gray-500">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3">Pilihan Hadiah</th>
                            <th scope="col" class="px-6 py-3 text-center">Poin Dibutuhkan</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($rewards as $reward)
                            <tr class="bg-white hover:bg-gray-50/50 transition-colors" wire:key="reward-{{ $reward->id }}">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        @if($reward->photo)
                                            <img src="{{ Storage::url($reward->photo) }}" class="h-12 w-12 flex-shrink-0 rounded-lg object-cover border border-gray-200" alt="{{ $reward->name }}">
                                        @else
                                            <div class="h-12 w-12 flex-shrink-0 rounded-lg bg-brand-light/20 flex items-center justify-center text-brand font-bold border border-brand-light">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            <div class="text-xs text-gray-500 truncate max-w-[250px]">{{ $reward->description ?? 'Tidak ada deskripsi' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="font-bold text-brand-dark flex flex-col items-center">
                                        <span>{{ number_format($reward->points_required, 0, ',', '.') }}</span>
                                        <span class="text-[10px] uppercase tracking-wider text-brand font-semibold">Poin</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="px-2.5 py-1 rounded-full text-xs font-medium {{ $reward->stock > 5 ? 'bg-blue-100 text-blue-800' : ($reward->stock > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2" x-data="{ openDeleteModal: false }">
                                        ...
                                        <button wire:click="toggleActive({{ $reward->id }})" wire:loading.attr="disabled"
                                            class="inline-flex items-center p-1.5 rounded-lg border {{ $reward->is_active ? 'border-red-200 text-red-600 hover:bg-red-50' : 'border-green-200 text-green-600 hover:bg-green-50' }}"
                                        ...
                                        <!-- Edit Button -->
                                        <button wire:click="edit({{ $reward->id }})" class="inline-flex items-center p-1.5 rounded-lg border border-blue-200 text-blue-600 hover:bg-blue-50" title="Edit">
                                                    ...
                                                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                        <div class="sm:flex sm:items-start">
                                                            ...
                                                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Hapus Hadiah</h3>
                                                                <div class="mt-2 text-wrap whitespace-normal">
                                                                    <p class="text-sm text-gray-500">Hapus hadiah <b>{{ $reward->name }}</b>? Pastikan tidak ada penukaran poin yang sedang diproses untuk hadiah ini.</p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                                        <button @click="openDeleteModal = false; $wire.delete({{ $reward->id }})" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">Hapus</button>
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm">Belum ada daftar hadiah penukaran.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                    <form wire:submit.prevent="save">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4 font-heading border-b pb-2" id="modal-title">{{ $rewardId ? 'Edit' : 'Tambah' }} Hadiah</h3>
                            
                            <div class="space-y-4">
                                <div>
                                    <x-input-label for="name" value="Nama Hadiah" />
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <x-input-label for="points_required" value="Poin Dibutuhkan" />
                                        <x-text-input wire:model="points_required" id="points_required" type="number" min="0" step="1" class="mt-1 block w-full" required />
                                        <x-input-error :messages="$errors->get('points_required')" class="mt-1 text-xs" />
                                    </div>
                                    <div>
                                        <x-input-label for="stock" value="Stok Barang" />
                                        <x-text-input wire:model="stock" id="stock" type="number" min="0" step="1" class="mt-1 block w-full" required />
                                        <x-input-error :messages="$errors->get('stock')" class="mt-1 text-xs" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <x-input-label for="photo" value="Foto Hadiah" />
                                    ...
                                    @elseif ($existingPhoto)
                                        <div class="mt-2">
                                            <img src="{{ Storage::url($existingPhoto) }}" class="h-20 w-auto rounded object-cover shadow border">
                                        </div>
                                    @endif
                                </div>
                            </div>
